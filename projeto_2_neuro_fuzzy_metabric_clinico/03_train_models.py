...
def run_experiment(df, *, include_survival_months: bool, output_name: str) -> pd.DataFrame:
    ...
    for name, estimator in build_model_registry().items():
        ...
        scenario_table = summarize_threshold_scenarios(y_test, y_prob)
        scenario_table.insert(0, "model", name)
        scenario_table.insert(1, "include_survival_months", include_survival_months)
        scenario_table.to_csv(TABLES_DIR / output_name.replace(".csv", f"__{name}_threshold_scenarios.csv"), index=False)
        rows.append(
            {
                "model": name,
